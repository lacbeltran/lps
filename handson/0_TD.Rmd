...
- My answer: I can see a dotted vertical line from the Min to the Max values, the median as a horizontal line in the middle of the boxplot, and the horizontal edges aroun the median are the first and the third quartile. This representation is useful to understand the distribution of the time values.
...
## Statistical interpretation for Real Ping-Pong Measurements (PP_size_1.csv data)
- Plot of all data

```{r}
plot(df1$time, ylab="Time (seconds)", xlab="Measurement Number", cex = .2);
```
An overview of the data shows that all the measurements are of less than one second.

- Statistical Summanry

```{r}
summary(df1$time);
```
The mean estimation is 4.732e-05, while the median gives 3.164e-05.
...
```
The boxplot shows several outilers.

- Histogram

```{r, fig.width=3}
hist(df1$time, breaks=5, xlab="Time (seconds)", main="Histogram of Real Ping-Pong")
```
The histogram confirms the idea that most of points are really close to zero.
...
- Plot of Geophysics Parallel Application

```{r}
plot(df$duration, ylab="Duration (seconds)", xlab="Iterations");
```

- Boxplot

```{r, fig.width=2, fig.height=3.5}
boxplot(df$duration, ylab="Duration (seconds)")
```
...
```{r, fig.width=6}
hist(df$duration, breaks=5, xlab="Iterations", main="Histogram of Geophysics Parallel Application")
```